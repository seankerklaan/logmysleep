...
const SleepDiary = () => {
  ...
  const loadSleepData = async () => {
    try {
      ...
      if (data?.sleep_data) {
        setSleepStates(data.sleep_data);
      }
    } catch (error) {
  ...
  // Wrap setSleepStates to save data after state updates
  const handleSleepStateChange = (newStates: SleepState) => {
    setSleepStates(newStates);
    saveSleepData(newStates);
  };
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <SleepGrid sleepStates={sleepStates} setSleepStates={handleSleepStateChange} />
          </div>
